...
def is_input_valid(text: str, min_length: int = 10, short_text_threshold: int = 20,
                   min_unique_chars_for_short_text: int = 3, allow_short_natural_lang: bool = False) -> bool:
    if not text or not text.strip(): return False
    stripped_text = text.strip()
    if allow_short_natural_lang and len(stripped_text) < min_length:
        return bool(len(stripped_text) >= 1 and any(c.isalnum() for c in stripped_text))
...
class ClientRepresentativeCreatorAgent:
    def __init__(self, verbose: bool = False):
        ...
        # --- ADDED: Load supplemental information ---
        self.alpine_info_path = "alpine_partners_and_christian_schmidt_summary_information.txt"  # Replace with actual path
        self.crazybirds_jd_path = "crazybirds_ceo_job_description.txt"  # Replace with actual path
    ...
    def _load_persona_info(self) -> str:
        try:
            with open(self.alpine_info_path, "r", encoding="utf-8") as f:
                return f.read().strip()
        except FileNotFoundError:
            logger.warning(f"{self.name}: Persona info file not found: {self.alpine_info_path}")
            return ""
        except Exception as e:
            logger.error(f"{self.name}: Error loading persona info: {e}")
            return ""

    def _load_job_description(self) -> str:
        try:
            with open(self.crazybirds_jd_path, "r", encoding="utf-8") as f:
                return f.read().strip()
        except FileNotFoundError:
            logger.warning(f"{self.name}: Job description file not found: {self.crazybirds_jd_path}")
            return ""
        except Exception as e:
            logger.error(f"{self.name}: Error loading job description: {e}")
            return ""

    def _create_prompt(self) -> None:
        template = """You are an expert at crafting AI agent personas, **but you MUST ONLY use the information provided to you.** Do not add information or make assumptions that are not explicitly mentioned in the provided context.

Your task is to create a persona based on a client description and optionally supplement it with tone information from files. **You are NOT creating an agent specifically for CrazyBirds or any other company unless explicitly instructed to do so in the client description.**

Here is the client description:
--- CLIENT DESCRIPTION START ---
{client_description}
--- CLIENT DESCRIPTION END ---
...
The following is SUPPLEMENTAL INFORMATION that *may* be relevant to the persona, but should only be used if it is explicitly relevant to the client description:
--- CHRISTIAN SCHMIDT PERSONA (SUPPLEMENTAL) ---
{persona_info}
--- CHRISTIAN SCHMIDT PERSONA END ---

--- CRAZYBIRDS JOB DESCRIPTION (SUPPLEMENTAL) ---
{job_description}
--- CRAZYBIRDS JOB DESCRIPTION END ---

Based ONLY on ALL the provided information (Client Description, Tone from Files, and SUPPLEMENTAL Persona/Job Description IF RELEVANT), write a single, compelling paragraph that serves as an AI agent's system prompt or internal monologue. This paragraph should:
- Embody the client's core persona: their mindset, primary concerns, and values.
- Capture the client's typical communication style: direct, pragmatic, analytical (if discernible from the tone files).
- **ONLY include details and considerations that are DIRECTLY supported by the provided information. If the client description and tone files provide insufficient information to create a meaningful persona, respond with the following EXACT TEXT: "Insufficient information to generate a meaningful persona."**
- Be written as if it's the client speaking or thinking to themselves, guiding their actions. It should be a natural internal narrative, NOT a list of instructions for the AI.

Generated Client Persona Prompt:
"""
        self.prompt_template = PromptTemplate(
            input_variables=["client_description", "client_tone_from_files", "persona_info", "job_description"],
            template=template,
        )
    ...
    def _extract_text_from_file(self, file_path: str) -> str:
        ...
        try:
            if ext == ".pdf":
                ...
                logger.warning(f"{self.name}: Unsupported file: {file_path}");
                return ""
            return content.strip()
        except Exception as e:
    ...
    def run(self, client_description: str, transcript_file_paths: Optional[List[str]] = None) -> str:
        ...
        # Allow short natural language for client_description. min_length 3 for "CEO" etc.
        is_desc_valid = final_client_desc and is_input_valid(final_client_desc, min_length=3,
                                                             allow_short_natural_lang=True)
        is_files_valid = client_tone_files and is_input_valid(client_tone_files,
                                                               min_length=20)  # Files should have more substance

        if not is_desc_valid and not is_files_valid:
            logger.error(f"{self.name}: Client desc and file content insufficient.")
            return "Error: Client description and transcript file content are both insufficient or non-meaningful."

        desc_for_llm = final_client_desc if is_desc_valid else "No explicit client description or it was too brief/non-meaningful."

        if self.verbose:
            logger.info(
                f"{self.name}: Desc for LLM: {desc_for_llm[:100]}..., Files text preview: {client_tone_files[:100]}...")
        ...
        try:
            if hasattr(self.chain, 'invoke'):
                ...
                response = res_dict.get('text', str(res_dict))
            else:
