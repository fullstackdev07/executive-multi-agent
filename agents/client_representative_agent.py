# import os, logging, re, json, fitz
...
"""
        self.extraction_template = PromptTemplate(
            input_variables=["user_input"],
            template=inject_guidance(template_body)
        )
    ...
    def run(self, user_input: str, files: Optional[List[str]] = None) -> str:
        ...
        try:
            ...
            if not extracted_json_str.startswith("{"):
                logger.warning(f"LLM response is not valid JSON. Raw response:\n{extracted_json_str}")
                return (
                    "\u26a0\ufe0f Sorry, I couldn’t understand the input or generate a valid client summary.\n"
                    "Please make sure the input is clearly described and try again.\n\n"
                    f"Raw response:\n{extracted_json_str}"
                )
            extracted_data = json.loads(extracted_json_str)
        except json.JSONDecodeError as e:
            logger.error(f"JSON decoding failed: {e} | Raw string: {extracted_json_str}")
            return (
                f"\u26a0\ufe0f Failed to extract structured data. Please rephrase the input.\n"
                f"Error: {e}\nRaw output:\n{extracted_json_str}"
            )
        except Exception as e:
            logger.error(f"Unexpected error during JSON parsing: {e}")
            return f"\u26a0\ufe0f Unexpected error during client trait extraction: {e}"
