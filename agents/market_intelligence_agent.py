...
- If "Company Location" is "Not Specified", acknowledge this lack of specific information in the Company Overview section where appropriate. Do not repeatedly state "Not Specified" throughout the report if the information is simply unavailable.
- If "Geography of Focus" is "Global", conduct your analysis on a global scale. Your section titles (e.g., Market Overview, Competitive Landscape, SWOT) should reflect this global context (e.g., "Market Overview (Global Focus)") or be phrased generally if "Global" is explicitly stated as the geography.
- If "Company Name" is generic (e.g., "Unknown Company", "A Company (Name Not Specified)"), acknowledge this. Focus the report on the general industry implied by supporting documents if possible, or clearly state that a specific company analysis is limited due to the lack of a specific company name.
- **Crucially, avoid using the literal word "Unknown" as a placeholder for missing company location or geography in your report's body or section titles.** Instead, adapt your language to reflect that the information is general, global, or not specified. For example, instead of "Market Overview for Unknown", if the geography is set to "Global", write "Market Overview (Global Focus)" or similar, and explain the scope in the text. If no specific geography was provided and it defaulted to "Global", state that the analysis takes a global perspective.
--- End of Important Instructions ---

Based on ALL the information above (company details, geography, supporting documents, and your own general knowledge), generate a comprehensive market intelligence report. The report should be well-structured and detailed.
...
2.  **Company Overview:** (Approx. 400-500 words)
    *   Detailed description of "{company_name}".
    *   Include available details on: revenue, number of employees, key office locations (if "{company_location}" is not "Not Specified", otherwise state location is not specified), main business lines/divisions, core products/services, significant historical milestones, and ownership structure (e.g., public, private, VC-backed).
    *   Synthesize information from provided documents and your general knowledge.
...
Ensure the report is professional, insightful, and directly addresses all sections, adhering to the instructions provided.
...
"""
    ...
    def run(self, input_data: dict) -> str:
        ...
        company_name_input = input_data.get("company_name", "A Company (Name Not Specified)") # More descriptive default
        supporting_docs = input_data.get("supporting_documents", "")

        # Validate company_name
        # Use a slightly different string for internal default to distinguish from user explicitly typing "Unknown"
        is_company_name_defaulted_or_truly_unknown = company_name_input == "A Company (Name Not Specified)" or \
                                                     company_name_input.lower() == "unknown"

        if is_company_name_defaulted_or_truly_unknown or \
           not is_input_valid(company_name_input, min_length=2, short_text_threshold=10, min_unique_chars_for_short_text=1):
            actual_company_name_for_report = "A Company (Name Not Specified)" if is_company_name_defaulted_or_truly_unknown else company_name_input
            logger.warning(f"MarketIntelligenceAgent: Company name '{actual_company_name_for_report}' is generic, 'Unknown', or invalid. Report generation might be impacted or fail if supporting documents are also insufficient.")
            if not (supporting_docs and is_input_valid(supporting_docs, min_length=50)):
                 return json.dumps({
                    "error": f"Company name '{actual_company_name_for_report}' is invalid, generic, or 'Unknown', and no substantial supporting documents were provided. Cannot generate report."
                })
            company_name_for_payload = actual_company_name_for_report
        else:
            company_name_for_payload = company_name_input


        # Validate supporting_documents if provided
        if supporting_docs and not is_input_valid(supporting_docs, min_length=50): # Combined docs should have some substance
            logger.warning(f"MarketIntelligenceAgent: Supporting documents appear non-meaningful or too short. Preview: {supporting_docs[:100]}...")
            if company_name_for_payload == "A Company (Name Not Specified)" or company_name_for_payload.lower() == "unknown":
                return json.dumps({
                    "error": "Company name is invalid/Unknown/generic and supporting documents are also non-meaningful. Cannot generate report."
                })
            supporting_docs_for_llm = "Note: Provided supporting documents were minimal or appeared non-meaningful."
        else:
            supporting_docs_for_llm = supporting_docs if supporting_docs else "No supporting documents provided."

        # Resolve company_location for prompt
        raw_company_location = input_data.get("company_location")
        if not raw_company_location or raw_company_location.strip().lower() == "unknown" or not raw_company_location.strip():
            company_location_for_payload = "Not Specified"
        else:
            company_location_for_payload = raw_company_location.strip()

        # Resolve geography for prompt
        raw_geography = input_data.get("geography")
        if not raw_geography or raw_geography.strip().lower() == "unknown" or not raw_geography.strip():
            geography_for_payload = "Global" # Default to Global if unknown or not provided
        else:
            geography_for_payload = raw_geography.strip()

        payload = {
            "role": self.role,
            "goal": self.goal,
            "company_name": company_name_for_payload,
            "company_location": company_location_for_payload,
            "geography": geography_for_payload,
            "supporting_documents": supporting_docs_for_llm,
        }

        if self.verbose:
            logger.info(f"MarketIntelligenceAgent: Payload for LLM: {payload}")
        ...
        try:
            if hasattr(self.chain, 'invoke'):
                ...
                response = response_dict.get('text')
                if response is None:
                    response = str(response_dict) # Fallback if 'text' key is missing
            else:
                 response = self.chain.run(payload)
        except Exception as e:
            logger.exception(f"MarketIntelligenceAgent: Error during LLM chain execution: {e}")
...
"""
        ...
        try:
            ...
            for key in final_details: # Ensure "Unknown" if empty or not valid
                # Allow "Unknown" as a valid extracted value, run() method will handle it for prompt.
                if not final_details[key] or not isinstance(final_details[key], str):
                    final_details[key] = "Unknown"
                elif final_details[key].lower() != "unknown" and \
                     not is_input_valid(final_details[key], min_length=2, short_text_threshold=10, min_unique_chars_for_short_text=1):
                    logger.warning(f"Extracted detail '{final_details[key]}' for '{key}' deemed invalid, defaulting to 'Unknown'.")
                    final_details[key] = "Unknown"
            if self.verbose:
                logger.info(f"MarketIntelligenceAgent: Extracted company details (parsed JSON): {final_details}")
            return final_details
